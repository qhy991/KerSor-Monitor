...
- **Legacy importer**: Existing `/workspace/repo/autokaggle` monitor state can be imported read-only from `tasks.json`, `monitor/state/bindings.tsv`, tmux panes, dashboard artifacts, and performance summaries. Imported workers are marked `managed_by=legacy` and `read_only=true`; `Latency` uses best latency in ms when available, and `MFU` is filled only when the source data exposes it.
