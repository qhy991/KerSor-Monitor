...
class StartWorkerDryRunTests(unittest.TestCase):
    def _make_infra(self) -> Path:
        ...
        shutil.copy(
            TEMPLATES / "worker-prompt-kersor.md", root / "templates" / "worker-prompt-kersor.md"
        )
    ...
    def _run(self, root: Path, *extra: str) -> subprocess.CompletedProcess:
        proc = subprocess.run(
            [
                "bash",
                str(root / "scripts" / "start-worker.sh"),
                "FI-001",
                "--engine",
                "kersor",
                "--dry-run",
        ...
        self.assertEqual(
            proc.returncode, 0, f"start-worker failed:\nstderr:{proc.stderr}\nstdout:{proc.stdout}"
        )
        return proc
    ...
    def test_combined_prompt_has_metadata_block_for_paper_run(self) -> None:
        ...
        combined = (
            root / "workspaces" / "fi_001_smoke" / "runs" / "combined_prompt.md"
        ).read_text()
...
class ArmTests(StartWorkerDryRunTests):
    ...
    def test_bestsingle_with_workflow_pins_single(self) -> None:
        ...
        self.assertEqual(
            self._status(root)["arm_flags"], "--workflows ako4x-kernel-optimizer --max-workflows 1"
        )
    ...
    def test_arm_on_non_kersor_engine_rejected(self) -> None:
        ...
        shutil.copy(
            TEMPLATES / "worker-prompt-kersor.md", root / "templates" / "worker-prompt-kda3phase.md"
        )
        proc = subprocess.run(
            [
                "bash",
    ...
    def test_max_dispatches_not_double_applied_for_bestsingle(self) -> None:
        root = self._make_infra()
        self._run(root, "--arm", "BestSingle", "--arm-workflow", "ako4x", "--max-dispatches", "5")
        # BestSingle already fixed --max-workflows 1; do not append a second one
        self.assertEqual(self._status(root)["arm_flags"], "--workflows ako4x --max-workflows 1")
